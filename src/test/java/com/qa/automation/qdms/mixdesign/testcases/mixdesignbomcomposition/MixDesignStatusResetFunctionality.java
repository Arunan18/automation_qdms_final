...
* Description
*------------
*Single Search Reset in Mix design Status 
***********************************************************************************
*
...
* ================       ====         =====        ========
* TC-MIX-DES-0029      01/05/2023    Karunya    Original Version
*
************************************************************************************/
...
public class MixDesignStatusResetFunctionality extends DriverIntialization {
	
	static  MixDesignPage mps = new MixDesignPage();
	 
	@Test
	public static void mixdesignStatusReset() throws InterruptedException, IOException {
		
		LoginTest.Login();
		
		MixDesignNavigation.mixDesignNavigation();
		
		MixDesignStatusResetFunctionality.mixdesignStatusResetButtonFunctionality();
		
	}
	
	public static void mixdesignStatusResetButtonFunctionality() throws InterruptedException, IOException {
		...
		PageFactory.initElements(driver, mps);
		 ...
		 boolean clickNextPage = true;
			...
			String PlantXpath_lastPart = "]/td[8]";
			while (clickNextPage) {
		 ...
		 Thread.sleep(3000);
			...
			// --------------------------------------------------------------------------------------
		      ...
		      MixdesignStatussearchFunctionality.mixdesignStatussearchIcon();
		 
		      MixdesignStatussearchFunctionality.mixdesignStatussearchTextbox();
		 
		      MixdesignStatussearchFunctionality.mixdesignStatussearchButton();
			...
			// --------------------------------------------------------------------------------------
		      ...
		      try {
		    	  ...
		    	  if ( MixDesignPage.StatussearchresetButton.isDisplayed()) {
						testCase = extent.createTest("STEP-01--STATUS RESET BUTTON IS DISPLAYED");
						testCase.log(Status.PASS, "TEST PASS");

						if ( MixDesignPage.StatussearchresetButton.isEnabled()) {
							testCase = extent.createTest("STEP-02--STATUS RESET BUTTON IS ENABLED");
							testCase.log(Status.PASS, "TEST PASS");
							Thread.sleep(3000);
							 MixDesignPage.StatussearchresetButton.click();

						} else {
							testCase = extent.createTest("STEP-02-STATUS RESET BUTTON IS NOT ENABLED");
							testCase.log(Status.PASS, "TEST FAIL");
						}
					} else {
						testCase = extent.createTest("STEP-01--STATUS RESET BUTTON IS NOT DISPLAYED");
						testCase.log(Status.PASS, "TEST FAIL");
					}
