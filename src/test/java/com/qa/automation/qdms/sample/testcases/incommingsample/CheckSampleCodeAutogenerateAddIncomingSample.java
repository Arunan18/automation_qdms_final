...
* Description
*------------
* Check Sample Code Autogenerate Add Incoming Sample
*****************************************************
*
* Author           : S.Quenthan
...
* ================       ====         =====        ========
*TC-INC-SAM-0166      10/05/2023   Quenthan     Orginal Version
*
************************************************************************************/
...
public class CheckSampleCodeAutogenerateAddIncomingSample extends DriverIntialization {
	@Test
	public static void autogenaratecheck() throws IOException, InterruptedException {
		

		IncomingSamplePage njk=new IncomingSamplePage();
		...
		PropertiesCommonMethods.excalRowCount("Samples Module.xlsx", "Incoming Sample", "TC-INC-SAM-0166-01");
		...
		while (nextPage2==true) {
			...
			for (int jl = 0; jl < rowe2; jl++) {
				nextPage2 = driver.findElement(By.xpath("//li[@title='Next Page']//button[@type='button']"))
						.isEnabled();
				String code = allrow2.get(jl).findElements(By.tagName("td")).get(9).getText();
				if (code.contains(datsD.get(1))) {
					String coder = allrow2.get(jl).findElements(By.tagName("td")).get(9).getText();
					search1.add(coder);
...
//					System.out.println("last number:-"+codeI);
		...
		ActionCommonMethod.actionS(0, 22, driver.findElement(By.xpath("//div[@class='ant-spin-container']")),"copy", "Samples Module.xlsx", "Incoming Sample", "TC-INC-SAM-0166-01", 1);
		
//		past data
		IncomingSamplePage.addIncomSample.click();
		
		Thread.sleep(1000);

		IncomingSamplePage.planttextbox.click();
...
//			System.out.println("rows:-"+rowe);
			for (int j = 0; j < rowe; j++) {
				...
				String sam = allrow.get(j).findElements(By.tagName("td")).get(9).getText();
