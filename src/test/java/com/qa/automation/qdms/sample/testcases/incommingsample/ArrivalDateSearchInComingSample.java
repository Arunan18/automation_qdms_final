...
//* Description
//*------------
//* Arrival date Search Functionality in Incoming Samples
//***********************************************************************************
//*
...
//* ================                      ====         =====        ========
//* TC-INC-SAM-0078, TC-INC-SAM-0080      20/04/2023   Sayan     Orginal Version
//* TC-INC-SAM-0082
//************************************************************************************
...
public class ArrivalDateSearchInComingSample extends DriverIntialization {
	
	static String SearchArrivalDate = null;
...
//		 THE METHOD IS FOR GENERATING REPORT 
	...
	public static void singleSearch() throws InterruptedException, IOException {
		...
		int firstrow = CommonMethods.getFirstRowNum(
				System.getProperty("user.dir") + "\\src\\test\\resources\\Excel\\Samples Module.xlsx",
				"Incoming Sample", "TC-INC-SAM-0082");
		...
		for (int i = firstrow; i <= lastrow; i++) {
			...
			SearchArrivalDate = (String) row.getCell(1).getStringCellValue();
...
//		        	STEP-03--INCOMING SAMPLE BUTTON IS DISPLAYED &	STEP-04--INCOMING SAMPLE BUTTON IS ENABLED
		...
		// **************************************************************************************************
//	       	STEP-05--ARRIVAL DATE SEARCH ICON IS DISPLAYED &	STEP-06--ARRIVAL DATE SEARCH ICON IS ENABLED
		// **************************************************************************************************
        ...
        try {
		  ...
		  for (int i = 0; i < 1000; i++) {
			 if (!pass) {
	             try {
	                 driver.findElement(
	                         By.xpath("//th[@class='ant-table-cell'][7]"))
	                         .click();
	                 pass = true;
	             } catch (Exception e) {
	             }
		...
		if (driver.findElement(By.xpath("//th[7]//div[1]//span[2]")).isDisplayed()) {
			testCase = extent.createTest("STEP-05--ARRIVAL DATE SEARCH ICON IS DISPLAYED");
			testCase.log(Status.PASS, "TEST PASS ☑");
			if (driver.findElement(By.xpath("//th[7]//div[1]//span[2]")).isEnabled()) {
				testCase = extent.createTest("STEP-06--ARRIVAL DATE SEARCH ICON IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS ☑");
				 
				driver.findElement(By.xpath("//th[7]//div[1]//span[2]")).click();
		...
		// ************************************************************************************************************
//	       	STEP-07--CHECKING ARRIVAL DATE SEARCH TEXTBOX DISPLAYED &	STEP-08--CHECKING ARRIVAL DATE SEARCH TEXTBOX ENABLED
		// ************************************************************************************************************

		if (driver.findElement(By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Arrival Date']"))
				.isDisplayed()) {
			testCase = extent.createTest("STEP-07--ARRIVAL DATE SEARCH BOX IS DISPLAYED");
			testCase.log(Status.PASS, "TEST PASS ☑");
			if (driver.findElement(By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Arrival Date']"))
					.isEnabled()) {
				testCase = extent.createTest("STEP-08--ARRIVAL DATE SEARCH BOX IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS ☑");
				try {
					driver.findElement(
							By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Arrival Date']"))
							.click();
					driver.findElement(
							By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Arrival Date']"))
							.sendKeys(SearchArrivalDate);
				...
				testCase = extent.createTest("STEP-08--ARRIVAL DATE SEARCH ICON IS ENABLED");
				testCase.log(Status.PASS, "TEST FAIL ❎");
			}
		} else {
			testCase = extent.createTest("STEP-07--ARRIVAL DATE SEARCH ICON IS DISPLAYED");
			testCase.log(Status.PASS, "TEST FAIL ❎");
		}
		...
		// ************************************************************************************************************
//	       	STEP-08--CHECKING ARRIVAL DATE SEARCH BUTTON DISPLAYED &	STEP-09--CHECKING ARRIVAL DATE SEARCH BUTTON ENABLED
		// ************************************************************************************************************
			...
			if (driver.findElement(By.xpath(
				...
				try {
					...
					String Code = driver.findElement(By.xpath(
							"//*[@id=\"root\"]/div/section/section/main/div/div/div[2]/div/div[1]/div/div/div/div/div[2]/div[2]/table/tbody/tr[2]/td[7]"))
							.getText();
		...
		while (clickNextPage) {
			for (int i = 2; i < 750; i++) {
				try {
					...
					if (!text.contains(SearchArrivalDate)) {
						passorfail = false;
					}
					if (text.contains(SearchArrivalDate)) {
