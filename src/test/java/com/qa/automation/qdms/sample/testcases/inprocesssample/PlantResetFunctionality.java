...
//		DESCRIPTION
//------------------
//					IN-PROCESS SAMPLE PLANT RESET TEST FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   19:04:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0047	 	 19:04:2023     @SASIKALA     Original Version
//* 
//*****************************************************************************************

/*
* Process Sample  "Plant"  Reset Button UI Functionality - TC-PRO-SAM-0047
*/
...
public class PlantResetFunctionality extends DriverIntialization {
...
// ***************In-Process Sample Plant Search Data Text Box Method ***************************************
	...
	@Test(priority = 4)
	public static void InProcessPlantSearchdataboxMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);
		...
		if (InprocessSamplePage.Plantsearchtextbox.isDisplayed()) {
			...
			if (InprocessSamplePage.Plantsearchtextbox.isEnabled()) {
				...
				try {
					...
					String testCaseId = "TC-PRO-SAM-0047";
					...
					for (int i = firstRow; i <= lastRow; i++) {
						...
						TestCasePrint("CONFIRM SEARCH DATA", SampleCodeSearchconfirmData, String.valueOf(Value));
						Thread.sleep(2000);
...
// ***************In-Process Sample Plant Reset button Method ****************************************
	@Test(priority = 5)
	public static void InProcessPlantResetbuttonMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);

		Thread.sleep(2000);
		if (InprocessSamplePage.Plantreseticon.isDisplayed()) {
			testCase = extent.createTest("INPROCESS PLANT RESET BUTTON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Plantreseticon.isEnabled()) {
				testCase = extent.createTest("INPROCESS PLANT RESET BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.Plantreseticon.click();
					Thread.sleep(2000);
				} catch (Exception e) {
		...
		String Values = InprocessSamplePage.Plantsearchtextbox.getAttribute("Value");
