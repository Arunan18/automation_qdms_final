...
//* Description
//*------------
//* Quantity Search Functionality in Incoming Samples
//***********************************************************************************
//*
...
//* ================                      ====         =====        ========
//* TC-INC-SAM-0150, TC-INC-SAM-0152      26/04/2023   Sayan     Orginal Version
//* TC-INC-SAM-0154
//************************************************************************************
...
public class QuantitySearchInComingSample extends DriverIntialization {
	
	
	static String SearchQuantity = null;
...
//				System.getProperty("user.dir") + "\\src\\test\\resources\\Excel\\Samples Module.xlsx",
//				"Incoming Sample", "TC-INC-SAM-0154");
...
//			SearchQuantity = (String) row.getCell(1).getStringCellValue();
...
//		        	STEP-03--INCOMING SAMPLE BUTTON IS DISPLAYED &	STEP-04--INCOMING SAMPLE BUTTON IS ENABLED
		...
		// ****************************************************************************************
//	       	STEP-05--QUANTITY SEARCH ICON IS DISPLAYED &	STEP-06--QUANTITY SEARCH ICON IS ENABLED
		// ****************************************************************************************
		...
		try {
			...
			for (int i = 0; i < 1000; i++) {
				if (!pass) {
					try {
						driver.findElement(By.xpath("//th[@class='ant-table-cell'][15]")).click();
						pass = true;
					} catch (Exception e) {
					}
				}
		...
		if (driver.findElement(By.xpath("//th[15]//div[1]//span[2]")).isDisplayed()) {
			testCase = extent.createTest("STEP-05--QUANTITY SEARCH ICON IS DISPLAYED");
			testCase.log(Status.PASS, "TEST PASS ☑");
			if (driver.findElement(By.xpath("//th[15]//div[1]//span[2]")).isEnabled()) {
				testCase = extent.createTest("STEP-06--QUANTITY SEARCH ICON IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS ☑");

				driver.findElement(By.xpath("//th[15]//div[1]//span[2]")).click();
		...
		// ************************************************************************************************************
//	       	STEP-07--CHECKING QUANTITY SEARCH TEXTBOX DISPLAYED &	STEP-08--CHECKING QUANTITY SEARCH TEXTBOX ENABLED
		// ************************************************************************************************************

		if (driver
				.findElement(By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Quantity & Brand Received']"))
				.isDisplayed()) {
			testCase = extent.createTest("STEP-07--QUANTITY SEARCH BOX IS DISPLAYED");
			testCase.log(Status.PASS, "TEST PASS ☑");
			if (driver
					.findElement(
							By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Quantity & Brand Received']"))
					.isEnabled()) {
				testCase = extent.createTest("STEP-08--QUANTITY SEARCH BOX IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS ☑");
				try {
					driver.findElement(
							By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Quantity & Brand Received']"))
				...
				testCase = extent.createTest("STEP-08--QUANTITY SEARCH ICON IS ENABLED");
				testCase.log(Status.PASS, "TEST FAIL ❎");
			}
		} else {
			testCase = extent.createTest("STEP-07--QUANTITY SEARCH ICON IS DISPLAYED");
			testCase.log(Status.PASS, "TEST FAIL ❎");
		}
		...
		// ************************************************************************************************************
//	       	STEP-08--CHECKING QUANTITY SEARCH BUTTON DISPLAYED &	STEP-09--CHECKING QUANTITY SEARCH BUTTON ENABLED
		// ************************************************************************************************************
			...
			if (driver.findElement(By.xpath(
				...
				try {
					...
					String Code = driver.findElement(By.xpath(
							"//*[@id=\"root\"]/div/section/section/main/div/div/div[2]/div/div[1]/div/div/div/div/div[2]/div[2]/table/tbody/tr[2]/td[15]"))
							.getText();
					System.out.println("PPPPP " + Code);
					String TextBox1 = driver
							.findElement(By.xpath(
									"//div[@class='ant-table-filter-dropdown']//input[@placeholder=' Quantity & Brand Received']"))
							.getAttribute("value");
		...
		while (clickNextPage) {
			for (int i = 2; i < 750; i++) {
				try {
					...
					if (!text.contains(SearchQuantity)) {
						passorfail = false;
					}
					if (text.contains(SearchQuantity)) {
