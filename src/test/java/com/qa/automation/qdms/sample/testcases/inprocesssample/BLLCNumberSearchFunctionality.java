...
//		DESCRIPTION
//------------------
//					IN-PROCESS SAMPLE BLLC NUMBER SEARCH  FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   27:04:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0131	 	 27:04:2023     @SASIKALA     Original Version
//* TC-PRO-SAM-0133
//* TC-PRO-SAM-0135
//*****************************************************************************************

/*
* Process Sample  "BLLC No"  Search Icon UI Functionality - TC-PRO-SAM-0131
* Process Sample  "BLLC No" Text Box UI Functionality - TC-PRO-SAM-0133
* Process Sample  "BLLC No"  Search Button UI Functionality -TC-PRO-SAM-0135 
*/
...
public class BLLCNumberSearchFunctionality extends DriverIntialization {
...
// ****************IN-PROCESS COUNT NUMBER OF ELEMENTS BEFORE SEARCH*****************
	...
	public static void SampleSearchCountBeforeMethod() throws InterruptedException {
		...
		String NameXpath_lastPart = " ]/td[13]";
		while (clickNextPage) {
...
// ***************IN-PROCESS SAMPLE "BLLC NUMBER" SEARCH ICON METHOD***********************
	...
	public static void InProcessVehicleNoSearchiconMethod() throws InterruptedException, IOException {
		...
		try {
			...
			for (int i = 0; i < 100; i++) {
				if (!pass) {
					try {
						InprocessSamplePage.Bllcsearchicon.click();
						pass = true;
					} catch (Exception e) {
					}
				}
		...
		if (InprocessSamplePage.Bllcsearchicon.isDisplayed()) {
			testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH ICON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Bllcsearchicon.isEnabled()) {
				testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH ICON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
			} else {
				testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH ICON ENABLED");
...
// **************IN-PROCESS SAMPLE "BLLC NUMBER" SEARCH SCREEN METHOD*************************
	...
	public static void InProcessVehicleNoSearchScreenMethod() throws InterruptedException, IOException {
		...
		Thread.sleep(1000);
		if (InprocessSamplePage.Bllcsearchpopup.isDisplayed()) {
			testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH SCREEN VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Bllcsearchpopup.isEnabled()) {
				testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH SCREEN ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.Bllcsearchpopup.click();
					Thread.sleep(2000);
				} catch (Exception e) {
				...
				testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH TEXTBOX ENABLED");
				testCase.log(Status.PASS, "TEST FAIL");
			}
		} else {
			testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH TEXTBOX VISIBLE");
			testCase.log(Status.FAIL, "TEST FAIL");
		}

	}

// *******************IN-PROCESS SAMPLE "BLLC NUMBER" SEARCH DATA TEXT BOX METHOD*************************
	...
	public static void InProcessVehicleNoSearchdataboxMethod() throws InterruptedException, IOException {
		...
		Thread.sleep(2000);
		if (InprocessSamplePage.Bllcsearchtextbox.isDisplayed()) {
			testCase = extent.createTest("BLLC NUMBER SEARCH TEXTBOX IS VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Bllcsearchtextbox.isEnabled()) {
				testCase = extent.createTest("BLLC NUMBER SEARCH TEXTBOX IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.Bllcsearchtextbox.click();

					String excelPath = System.getProperty("user.dir")
							+ "\\src\\test\\resources\\Excel\\Samples Module.xlsx";
					String excelSheetName = "Process Sample";
					String testCaseId = "TC-PRO-SAM-0131";
					...
					for (int i = firstRow; i <= lastRow; i++) {
						...
						InprocessSamplePage.Bllcsearchtextbox.sendKeys(SampleCodeSearchData);
						String Value = InprocessSamplePage.Bllcsearchtextbox.getAttribute("value");
						TestCasePrint("CONFIRM BLLC NUMBER SEARCH", String.valueOf(SampleCodeSearchconfirmData),
			...
			testCase = extent.createTest("BLLC NUMBER SEARCH TEXTBOX VISIBLE");
			testCase.log(Status.FAIL, "TEST FAIL");
		}

	}

// ***************IN-PROCESS SAMPLE "BLLC NUMBER" SEARCH BUTTON METHOD*****************
	@Test(priority = 5)
	public static void InProcessVehicleNoSearchbuttonMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);

		Thread.sleep(3000);
		if (InprocessSamplePage.Bllcsearchbutton.isDisplayed()) {
			testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH BUTTON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Bllcsearchbutton.isEnabled()) {
				testCase = extent.createTest("INPROCESS BLLC NUMBER SEARCH BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.Bllcsearchbutton.click();
					Thread.sleep(2000);
				} catch (Exception e) {
...
// ***************IN-PROCESS SAMPLE "BLLC NUMBER" SEARCH AFTER COUNT METHOD****************
