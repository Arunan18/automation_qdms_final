...
//* Description
//*------------
//* Sample Time Search Functionality in Incoming Samples
//***********************************************************************************
//*
...
//* ================                      ====         =====        ========
//* TC-INC-SAM-0123, TC-INC-SAM-0125      24/04/2023   Sayan     Orginal Version
//* TC-INC-SAM-0127
//************************************************************************************
...
public class SampleTimeSearchInComingSample extends DriverIntialization {
	
	static String SearchSampleTime = null;
...
//		 THE METHOD IS FOR GENERATING REPORT 
	...
	public static void singleSearch() throws InterruptedException, IOException {
		...
		int firstrow = CommonMethods.getFirstRowNum(
				System.getProperty("user.dir") + "\\src\\test\\resources\\Excel\\Samples Module.xlsx",
				"Incoming Sample", "TC-INC-SAM-0127");
		...
		for (int i = firstrow; i <= lastrow; i++) {
			...
			SearchSampleTime = (String) row.getCell(1).getStringCellValue();
...
//		        	STEP-03--INCOMING SAMPLE BUTTON IS DISPLAYED &	STEP-04--INCOMING SAMPLE BUTTON IS ENABLED
		...
		// ****************************************************************************************
//	       	STEP-05--SAMPLE TIME SEARCH ICON IS DISPLAYED &	STEP-06--SAMPLE TIME SEARCH ICON IS ENABLED
		// ****************************************************************************************
		...
		try {
			...
			for (int i = 0; i < 1000; i++) {
				if (!pass) {
					try {
						driver.findElement(By.xpath("//th[@class='ant-table-cell'][12]")).click();
						pass = true;
					} catch (Exception e) {
					}
				}
		...
		if (driver.findElement(By.xpath("//th[12]//div[1]//span[2]")).isDisplayed()) {
			testCase = extent.createTest("STEP-05--SAMPLE TIME SEARCH ICON IS DISPLAYED");
			testCase.log(Status.PASS, "TEST PASS ☑");
			if (driver.findElement(By.xpath("//th[12]//div[1]//span[2]")).isEnabled()) {
				testCase = extent.createTest("STEP-06--SAMPLE TIME SEARCH ICON IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS ☑");

				driver.findElement(By.xpath("//th[12]//div[1]//span[2]")).click();
		...
		// ************************************************************************************************************
//	       	STEP-07--CHECKING SAMPLE TIME SEARCH TEXTBOX DISPLAYED &	STEP-08--CHECKING SAMPLE TIME SEARCH TEXTBOX ENABLED
		// ************************************************************************************************************

		if (driver
				.findElement(By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder='  Sample Time']"))
				.isDisplayed()) {
			testCase = extent.createTest("STEP-07--SAMPLE TIME SEARCH BOX IS DISPLAYED");
			testCase.log(Status.PASS, "TEST PASS ☑");
			if (driver
					.findElement(
							By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder='  Sample Time']"))
					.isEnabled()) {
				testCase = extent.createTest("STEP-08--SAMPLE TIME SEARCH BOX IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS ☑");
				try {
					driver.findElement(
							By.xpath("//div[@class='ant-table-filter-dropdown']//input[@placeholder='  Sample Time']"))
				...
				testCase = extent.createTest("STEP-08--SAMPLE TIME SEARCH ICON IS ENABLED");
				testCase.log(Status.PASS, "TEST FAIL ❎");
			}
		} else {
			testCase = extent.createTest("STEP-07--SAMPLE TIME SEARCH ICON IS DISPLAYED");
			testCase.log(Status.PASS, "TEST FAIL ❎");
		}
		...
		// ************************************************************************************************************
//	       	STEP-08--CHECKING SAMPLE TIME SEARCH BUTTON DISPLAYED &	STEP-09--CHECKING SAMPLE TIME SEARCH BUTTON ENABLED
		// ************************************************************************************************************
			...
			if (driver.findElement(By.xpath(
				...
				try {
					...
					String Code = driver.findElement(By.xpath(
							"//*[@id=\"root\"]/div/section/section/main/div/div/div[2]/div/div[1]/div/div/div/div/div[2]/div[2]/table/tbody/tr[2]/td[12]"))
							.getText();
					System.out.println("PPPPP " + Code);
					String TextBox1 = driver
							.findElement(By.xpath(
									"//div[@class='ant-table-filter-dropdown']//input[@placeholder='  Sample Time']"))
							.getAttribute("value");
		...
		while (clickNextPage) {
			for (int i = 2; i < 750; i++) {
				try {
					...
					if (!text.contains(SearchSampleTime)) {
						passorfail = false;
					}
					if (text.contains(SearchSampleTime)) {
