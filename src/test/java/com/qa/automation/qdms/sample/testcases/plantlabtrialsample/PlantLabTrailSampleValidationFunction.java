...
public class PlantLabTrailSampleValidationFunction extends DriverIntialization {
	...
	public static void excel() throws InterruptedException, IOException {
		...
		String testCaseId = "TC-P-LAB-SAM-0243-TD";
	...
	public static void validation() throws InterruptedException, IOException {
		...
		String excelPath = System.getProperty("user.dir") + "\\src\\test\\resources\\Excel\\Samples Module.xlsx";
		String excelSheetName = "Plant Lab Trial Sample";
		String testCaseId = "TC-P-LAB-SAM-0243";
		...
		for (int i = firstRow; i <= lastRow; i++) {
			...
			try {
				...
				if (PlantLabTrailSamplePage.testDate.isDisplayed()) {
					testCase = extent.createTest("THE TEST DATE IS DISPLAYED");
					testCase.log(Status.PASS, "TEST PASS");
					if (PlantLabTrailSamplePage.testDate.isEnabled()) {
						testCase = extent.createTest("THE TEST DATE IS ENABLED");
						testCase.log(Status.PASS, "TEST PASS");
						PlantLabTrailSamplePage.testDate.sendKeys(testDate);
						Thread.sleep(2000);
						PlantLabTrailSamplePage.testDate.sendKeys(Keys.ENTER);
					} else {
			...
			Thread.sleep(2000);
			try {
				if (PlantLabTrailSamplePage.noOfTrailTextBox.isDisplayed()) {
					testCase = extent.createTest("THE NO OF TRAIL IS DISPLAYED");
					testCase.log(Status.PASS, "TEST PASS");
					if (PlantLabTrailSamplePage.noOfTrailTextBox.isEnabled()) {
						testCase = extent.createTest("THE NO OF TRAIL IS ENABLED");
						testCase.log(Status.PASS, "TEST PASS");
						PlantLabTrailSamplePage.noOfTrailTextBox.click();
						Thread.sleep(2000);
						PlantLabTrailSamplePage.noOfTrailTextBox.sendKeys(noOfTrail, Keys.ENTER);
					} else {
			...
			PlantLabTrailSamplePage.submitButton.click();
			if (PlantLabTrailSamplePage.submitButton.isDisplayed()) {
				if (check.contentEquals("date")) {
					try {
						testCase = extent.createTest(forReportDate);
						String ActualValidation = PlantLabTrailSamplePage.testDateValidation.getText();
				...
				if (check.contentEquals("number")) {
					try {
						...
						String ActualValidation = PlantLabTrailSamplePage.noOfTrailValidation.getText();
