...
//		DESCRIPTION
//------------------
//					IN-PROCESS SAMPLE CODE RESET TEST FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   18:04:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0029	 	 18:04:2023     @SASIKALA     Original Version
//* 
//*****************************************************************************************

/*
* Process Sample  "Code"  Reset Button UI Functionality - TC-PRO-SAM-0029
*/
...
public class CodeResetFunctionality extends DriverIntialization {
...
// ***************In-Process Sample Code Search Data Text Box Method ***************************************
	...
	public static void InProcessCodeSearchdataboxMethod() throws InterruptedException, IOException {
		...
		if (InprocessSamplePage.Codesearchtextbox.isDisplayed()) {
			testCase = extent.createTest("CUSTOMER SEARCH TEXTBOX IS VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Codesearchtextbox.isEnabled()) {
				testCase = extent.createTest("CUSTOMER SEARCH TEXTBOX IS ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					...
					String testCaseId = "TC-PRO-SAM-0029";
					...
					for (int i = firstRow; i <= lastRow; i++) {
						...
						TestCasePrint("CONFIRM SEARCH DATA INSIDE", SampleCodeSearchconfirmData, String.valueOf(Value));
						Thread.sleep(2000);
			...
			testCase = extent.createTest("CUSTOMER SEARCH TEXTBOX VISIBLE");
			testCase.log(Status.FAIL, "TEST FAIL");
		}

	}

// ***************In-Process Sample Code Reset button Method ****************************************
	@Test(priority = 5)
	public static void InProcessCodeResetbuttonMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);

		Thread.sleep(2000);
		if (InprocessSamplePage.Codereseticon.isDisplayed()) {
			testCase = extent.createTest("INPROCESS CODE RESET BUTTON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.Codereseticon.isEnabled()) {
				testCase = extent.createTest("INPROCESS CODE RESET BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.Codereseticon.click();
					Thread.sleep(2000);
				} catch (Exception e) {
		...
		String Values = InprocessSamplePage.Codesearchtextbox.getAttribute("Value");
