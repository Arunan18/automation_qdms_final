...
//		DESCRIPTION
//------------------
//					IN-PROCESS SAMPLE STATUS RESET TEST FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   28:04:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0164	 	 28:04:2023     @SASIKALA     Original Version
//* 
//*****************************************************************************************

/*
* Process Sample  "STATUS"  Reset Button UI Functionality - TC-PRO-SAM-0164
*/
...
public class StatusResetFunctionality extends DriverIntialization {
...
// ************************************Excel Method**********************
	...
	public static void ExcelMethod() throws InterruptedException, IOException {
		...
		String testCaseId = "TC-PRO-SAM-0164";
...
// ****************Count Number of elements Before search**************
	...
	public static void InProcessSubCategorySearchScreenMethod() throws InterruptedException, IOException {
		...
		if (InprocessSamplePage.STATUSsearchpopup.isDisplayed()) {
			testCase = extent.createTest("INPROCESS STATUS SEARCH SCREEN VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.STATUSsearchpopup.isEnabled()) {
				testCase = extent.createTest("INPROCESS STATUS SEARCH SCREEN ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
		...
		if (InprocessSamplePage.STATUSsearchtextbox.isDisplayed()) {
			...
			if (InprocessSamplePage.STATUSsearchtextbox.isEnabled()) {
				...
				try {
					...
					InprocessSamplePage.STATUSsearchtextbox.sendKeys(SampleDateSearchData);
					Thread.sleep(1000);
...
// ***************In-Process Sample STATUS Reset button Method ****************************************
	@Test(priority = 5)
	public static void InProcessCodeResetbuttonMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);

		Thread.sleep(2000);
		if (InprocessSamplePage.STATUSreseticon.isDisplayed()) {
			testCase = extent.createTest("INPROCESS STATUS RESET BUTTON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.STATUSreseticon.isEnabled()) {
				testCase = extent.createTest("INPROCESS STATUS RESET BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					Actions actions = new Actions(driver);
					actions.sendKeys(Keys.ENTER).perform();
					InprocessSamplePage.STATUSreseticon.click();
					Thread.sleep(2000);
				} catch (Exception e) {
		...
		String Values = InprocessSamplePage.STATUSreseticon.getAttribute("Value");
