...
//		DESCRIPTION
//------------------
//					IN-PROCESS TRAIL SAMPLE NO OF TRAIL FILED CHECK FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   15:05:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0301  	   15:05:2023     @SASIKALA     Original Version
//****************************************************************************************************************

/*
* TC-PRO-SAM-0301	Check when do the Test trial Check the No of Trial Filed Functionality
*/
...
public class InProcessTrialNoOFTrailCheckFunctionality extends DriverIntialization{
static InprocessSamplePage InProPg = new InprocessSamplePage();
	...
	// CHECKNIG FINISH PRODUCT MENU IS DISPLAYED && STEP-02 CHECKNIG RAW MATERIAL MENU IS ENABLED
		SampleActionMethods.clicking(SampleNavigationPage.rawMaterialMainXpath(), 20, "STEP 2 - CHECKING RAW MATERIAL MENU");

    // CHECKNIG PLANT LAB TRIAL SAMPLE BUTTON IS DISPLAYED && STEP-03 CHECKNIG IN PROCESS SAMPLE BUTTON IS ENABLED
		SampleActionMethods.clicking(SampleNavigationPage.inprocessSampleXpath(), 20, "STEP 3 - CHECKING IN PROCESS SAMPLE BUTTON");
	...
	public static void EXCELWORK() throws InterruptedException, IOException {
		...
		int firstrow = CommonMethods.getFirstRowNum(
				System.getProperty("user.dir") + "\\src\\test\\resources\\Excel\\Samples Module.xlsx",
				"Process Sample", "TC-PRO-SAM-0301");
		...
		for (int i = firstrow; i <= lastrow; i++) {
			...
			NoOfTrail = (int) row.getCell(3).getNumericCellValue();
 ...
 // CLICK ON THE TEST	
	...
	public static void TESTSELECTION() throws InterruptedException, IOException {
		...
		driver.findElement(By
				.xpath("//div[@class='ant-ribbon-wrapper']//div[@class='sc-gzVnrw KZmpn']//li[text()='" + TestName + "']"))
				.click();
...
//	CHECK THE FUNCTIONALITY OF NO OF TRAIL FIELD
	...
	if (driver.findElement(By.id("no_of_trial")).isDisplayed()) {
		testCase = extent.createTest("NO OF TRAIL FIELD IS DISPLAYED");
		testCase.log(Status.PASS, "TEST PASS â˜‘");

		if (driver.findElement(By.id("no_of_trial")).isEnabled()) {
			testCase = extent.createTest("NO OF TRAIL FIELD IS ENABLED");
			testCase.log(Status.PASS, "TEST PASS â˜‘");

			driver.findElement(By.id("no_of_trial")).click();
			driver.findElement(By.id("no_of_trial")).sendKeys(String.valueOf(NoOfTrail));
...
//VERIFY THE DATE FIELD FUNCTIONALITY
	...
	Thread.sleep(1000);
	String ActualData = driver.findElement(By.id("no_of_trial")).getAttribute("value");
	int ExpectedData = NoOfTrail;
	System.out.println("No Of Trail : " + ActualData);
	testCase = extent.createTest("VERIFY THE NO OF TRAIL FUNCTIONALITY");
	if (ActualData.equals(String.valueOf(NoOfTrail))) {
