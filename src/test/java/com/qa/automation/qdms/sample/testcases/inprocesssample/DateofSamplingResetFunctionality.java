...
//		DESCRIPTION
//------------------
//					IN-PROCESS SAMPLE DATE OF SAMPLING RESET TEST FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   28:04:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0092	 	 28:04:2023     @SASIKALA     Original Version
//* 
//*****************************************************************************************

/*
* Process Sample  "DATE OF SAMPLING"  Reset Button UI Functionality - TC-PRO-SAM-0092
*/
...
public class DateofSamplingResetFunctionality extends DriverIntialization{
...
// ***************************Reach In-Process Sample**************
	...
	//******************Excel Method**********************
		...
		String testCaseId = "TC-PRO-SAM-0092";
	...
	// ***************In-Process Sample DATE OF SAMPLING Search Icon Method *****************************************
		...
		public static void InProcessSubCategorySearchiconMethod() throws InterruptedException, IOException {
			...
			if (InprocessSamplePage.DOSsearchicon.isDisplayed()) {
				...
				if (InprocessSamplePage.DOSsearchicon.isEnabled()) {
					testCase = extent.createTest("INPROCESS DATE OF SAMPLING SEARCH ICON ENABLED");
					testCase.log(Status.PASS, "TEST PASS");
		...
		public static void InProcessSubCategorySearchScreenMethod() throws InterruptedException, IOException {
			...
			if (InprocessSamplePage.DOSsearchpopup.isDisplayed()) {
				testCase = extent.createTest("INPROCESS DATE OF SAMPLING SEARCH SCREEN VISIBLE");
				testCase.log(Status.PASS, "TEST PASS");
				if (InprocessSamplePage.DOSsearchpopup.isEnabled()) {
					testCase = extent.createTest("INPROCESS DATE OF SAMPLING SEARCH SCREEN ENABLED");
					testCase.log(Status.PASS, "TEST PASS");
					try {
			...
			if (InprocessSamplePage.DOSsearchtextbox.isDisplayed()) {
				...
				if (InprocessSamplePage.DOSsearchtextbox.isEnabled()) {
					...
					try {
						...
						SelectDate.dateSelect("Samples Module.xlsx", "Process Sample", "TC-PRO-SAM-0092-01", 1,InprocessSamplePage.DOSsearchtextbox);
						Thread.sleep(1000);
...
// ***************In-Process Sample DATE OF SAMPLING Reset button Method ****************************************
	@Test(priority = 5)
	public static void InProcessCodeResetbuttonMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);

		Thread.sleep(2000);
		if (InprocessSamplePage.DOSreseticon.isDisplayed()) {
			testCase = extent.createTest("INPROCESS DATE OF SAMPLING RESET BUTTON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.DOSreseticon.isEnabled()) {
				testCase = extent.createTest("INPROCESS DATE OF SAMPLING RESET BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.DOSreseticon.click();
					Thread.sleep(2000);
				} catch (Exception e) {
		...
		String Values = InprocessSamplePage.DOSreseticon.getAttribute("Value");
