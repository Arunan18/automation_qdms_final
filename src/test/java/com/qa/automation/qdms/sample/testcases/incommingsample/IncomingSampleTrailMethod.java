...
public class IncomingSampleTrailMethod extends DriverIntialization {
	...
	public static void excel() throws InterruptedException, IOException {
		...
		String testCaseId = "TC-INC-SAM-0168-TD";
...
//VALIDATION CHECKING
	public static void validation() throws InterruptedException, IOException {
		...
		String excelPath = System.getProperty("user.dir") + "\\src\\test\\resources\\Excel\\Samples Module.xlsx";
		String excelSheetName = "Incoming Sample";
		String testCaseId = "TC-INC-SAM-0168";
		...
		for (int i = firstRow; i <= lastRow; i++) {
			...
			try {
				...
				IncomingSamplePage.dateCloseIcon.click();
				IncomingSamplePage.testDate.sendKeys(testDate, Keys.ENTER);
			} catch (NoSuchElementException e) {
				// TODO: handle exception
			}
			Thread.sleep(2000);
			try {
				IncomingSamplePage.noOfTrailTextBox.click();
				Thread.sleep(2000);
				IncomingSamplePage.noOfTrailTextBox.sendKeys(noOfTrail, Keys.ENTER);
			} catch (NoSuchElementException e) {
				// TODO: handle exception
			}
			Thread.sleep(2000);
			...
			if (IncomingSamplePage.submitButton.isDisplayed()) {
				if (check.contentEquals("date")) {
					try {
						testCase = extent.createTest(forReportDate);
						String ActualValidation = IncomingSamplePage.testDateValidation.getText();
				...
				if (check.contentEquals("number")) {
					try {
						...
						String ActualValidation = IncomingSamplePage.noOfTrailValidation.getText();
