...
//		DESCRIPTION
//------------------
//					IN-PROCESS SAMPLE SAMPLE CODE RESET TEST FUNCTIONALITY
//							Author :-		 @SASIKALA AMBALAVANAR
//							DATE Written:-   26:04:2023	
//****************************************************************************************
//* Test Case Number      	Date          Intis        Comments
//* =================       ======      ========       ========
//* TC-PRO-SAM-0119	 	 26:04:2023     @SASIKALA     Original Version
//* 
//*****************************************************************************************

/*
* Process Sample  "SAMPLE CODE"  Reset Button UI Functionality - TC-PRO-SAM-0119
*/
...
public class SamplecodeResetFunction extends DriverIntialization {
...
// ***************In-Process Sample SAMPLE CODE Search Screen Method ****************************************
		...
		if (InprocessSamplePage.SampleCodesearchtextbox.isDisplayed()) {
			...
			if (InprocessSamplePage.SampleCodesearchtextbox.isEnabled()) {
				...
				try {
					...
					String testCaseId = "TC-PRO-SAM-0119";
					...
					for (int i = firstRow; i <= lastRow; i++) {
						...
						TestCasePrint("CONFIRM SAMPLE CODE SEARCH DATA", SampleCodeSearchconfirmData,
...
// ***************In-Process Sample SAMPLE CODE Reset button Method ****************************************
	@Test(priority = 5)
	public static void InProcessCodeResetbuttonMethod() throws InterruptedException, IOException {
		PageFactory.initElements(driver, inprocesssample);

		Thread.sleep(2000);
		if (InprocessSamplePage.SampleCodereseticon.isDisplayed()) {
			testCase = extent.createTest("INPROCESS SAMPLE CODE RESET BUTTON VISIBLE");
			testCase.log(Status.PASS, "TEST PASS");
			if (InprocessSamplePage.SampleCodereseticon.isEnabled()) {
				testCase = extent.createTest("INPROCESS SAMPLE CODE RESET BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				try {
					InprocessSamplePage.SampleCodereseticon.click();
					Thread.sleep(2000);
				} catch (Exception e) {
		...
		String Values = InprocessSamplePage.SampleCodereseticon.getAttribute("Value");
		TestCasePrint("CONFIRM RESET", "null", String.valueOf(Values));
		Thread.sleep(2000);
	}

// ***************In-Process Sample SAMPLE CODE Search After Count Method****************************************
