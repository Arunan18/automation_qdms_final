...
//* Description
//*------------
//* ReviewTestParameterCancel-EditFunction_BeforeDepend
//***********************************************************************************
//*
...
//* ================       ====         =====        ========
//*TC-TCN-0121           8/05/2023      Niluxy      Original Version
//*
//************************************************************************************
...
public class ReviewTestParameterCancelEditFunction extends DriverIntialization {
	...
	public static void testparameter() throws InterruptedException, IOException {
		...
		extent.createTest("TC-TCN-0121:ReviewTestParameterCancel-EditFunction_BeforeDepend");
		// LOGIN TO THE SYSTEM
		LoginTest.Login();
		...
		AddGroupKey.actionS(0, 14, ManageTestsPage.managetexttablecontainer, "Description",
				"Test Configuragtion Module.xlsx", "Test_Configure", "TC-TCN-0121-01", 9);
...
//FIND THE EDIT ICON ELEMENT FROM THE EXCEL
		...
		// 15:CHECKING THE CANCEL BUTTON ICON ");
		Cancelbuttonreviewtestinedit();

		/* CHECK FIRST DATA OF THE TABLE */
		Thread.sleep(1000);
		SampleCommonMethods.firstrowdataCancel("STEP 18: EDITED DATA ADDED IN FIRST ROW", TestConfigPage.tablefirstrow,
				parameter);
...
//METHOD FOR RELEVENT CHECKBOX//

	public static void releventcheckbox() throws InterruptedException {
		Thread.sleep(2000);
		if (TestConfigPage.releventcheckboxreviewtest.isDisplayed()) {
			testCase = extent.createTest("STEP 13:RELEVENT CHECK BOX DISPLYED");
			testCase.log(Status.PASS, "TEST PASS");

			if (TestConfigPage.releventcheckboxreviewtest.isEnabled()) {
				testCase = extent.createTest("STEP 13:RELEVENT CHECK BOX ENABLED");
				testCase.log(Status.PASS, "TEST PASS");

				if (releventValue = true) {

					TestConfigPage.releventcheckboxreviewtest.click();
					testCase = extent.createTest("STEP 13:RELEVENT CHECK BOX CLICKED");
					testCase.log(Status.PASS, "TEST PASS");

				} else {
					testCase = extent.createTest("STEP 13:RELEVENT CHECK BOX IS NOT CLICKED");
					testCase.log(Status.FAIL, "TEST FAIL");
				}
			} else {
				testCase = extent.createTest("STEP 13:RELEVENT CHECK BOX NOT ENABLED");
				testCase.log(Status.FAIL, "TEST FAIL");
			}
		} else {
			testCase = extent.createTest("STEP 13:RELEVENT CHECK BOX NOT DISPLYED");
			testCase.log(Status.FAIL, "TEST FAIL ❎");
		}
	}

	// METHOD FOR CANCEL BUTTON IN EDIT FUNCTION//
	public static void Cancelbuttonreviewtestinedit() throws InterruptedException {

		Thread.sleep(3000);
		if (TestConfigPage.cancelbuttonreviewtest.isDisplayed()) {

			testCase = extent.createTest("STEP 14:REVIEWTEST EDITFUNCTION- CANCEL BUTTON DISPLY");
			testCase.log(Status.PASS, "TEST PASS");
			if (TestConfigPage.cancelbuttonreviewtest.isEnabled()) {
				testCase = extent.createTest("STEP 15:REVIEWTEST EDITFUNCTION- CANCEL BUTTON ENABLED");
				testCase.log(Status.PASS, "TEST PASS");
				...
				try {
					...
					if (clicked) {
						testCase = extent.createTest("STEP 16:REVIEWTEST EDITFUNCTION- CANCEL BUTTON CLICKED");
						testCase.log(Status.PASS, "TEST PASS");
						...
						testCase = extent.createTest("STEP 16:REVIEWTEST EDITFUNCTION- CANCEL BUTTON NOT CLICKED");
						testCase.log(Status.FAIL, "TEST FAIL");
					}
				...
				testCase = extent.createTest("STEP 15:REVIEWTEST EDITFUNCTION- CANCEL BUTTON IS ENABLED");
				testCase.log(Status.FAIL, "TEST FAIL");
			}
		} else {
			testCase = extent.createTest("STEP 14:REVIEWTEST EDITFUNCTION- CANCEL BUTTON DISPLY");
